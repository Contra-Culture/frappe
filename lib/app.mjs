export function App(name, document, setupWidgets, setupScreens, setupRoutes) {
  ...
  const sfGen = (screen, data, chunkMatcher, setupChildren) => {
    const sfGen = function(screen, data, chunkMatcher, setupChildren) {
      const children = []
      if ( setupChildren !== null ) {
        const node = (screen, data, chunkMatcher, setupChildren) => {
          children.push(sfGen(screen, data, chunkMatcher, setupChildren))
        }
        setupChildren(node)
      }
    ...
    screenFinder = sfGen(screen, data, chunkMatcher, setupChildren);
  }
