...
const loader = function(window) {
  ...
  const load = (path, {title, src, async, defer }) => {
    ...
    scriptElem.createAttribute("title", title)
    scriptElem.createAttribute("src", src)
    scriptElem.createAttribute("async", async)
    scriptElem.createAttribute("defer", defer)
    scriptElem.createAttribute("type", "module")
