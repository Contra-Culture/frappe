...
const Registry = function({ validator, notFoundHandler, alreadyDefinedHandler, notValidHandler, plugins }) {
  ...
  this.getValues = (path, pluginPayload) => {
    if (path.length === 0) {
  ...
  this.getValue = (path, pluginPayload) => {
    const [chunk, ...restChunks] = path
    if (restChunks.length === 0) {
  ...
  this.setValue = (path, value, pluginPayload) => {
