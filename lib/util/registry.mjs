...
const registry = ({ validator, notFoundHandler, alreadyDefinedHandler, notValidHandler, plugins }) => {
  ...
  const hasPath = (path) => {
    ...
    if (restChunks.length === 0) {
      return values.has(chunk)
    } else {
      const child = children.get(chunk)
      if (child !== undefined) {
  ...
  const setValue = (path, value, pluginPayload) => {
    ...
    const [chunk, ...restChunks] = path
    if (restChunks.length === 0) {
      const __value = values.get(chunk)
      if (__value !== undefined) {
        return handleAlreadyDefined(path, value)
      }
      values.set(chunk, value)
    } else {
      let child = children.get(chunk)
      if (child === undefined) {
        child = registry({
          validator,
          notFoundHandler,
